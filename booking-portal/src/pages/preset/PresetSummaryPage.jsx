...
export default function PresetSummaryPage() {
  ...
  // Use tierPrice from step2 (not sum of item prices)
  const tierPrice = step2?.tierPrice || 0
  const guestCount = step1?.guestCount || 0
  const baseTotal = tierPrice * guestCount
  ...
  async function handleConfirm() {
    ...
    try {
      const payload = {
        ...
        pricePerPerson: tierPrice,  // explicit tier price override
      }
      ...
      navigate(`/success?type=booking&id=${data.bookingId}`)
    } catch (err) {
  ...
  return (
    <div className="booking-page">
      ...
      <div className="booking-body">
        {/* Package badge */}
        <div style={{ padding: '10px 16px', background: 'rgba(212,175,55,0.08)', border: '1px solid rgba(212,175,55,0.25)', borderRadius: 10, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 10 }}>
          <i className="fa-solid fa-box-open" style={{ color: 'var(--gold)' }} />
          <span style={{ color: 'var(--text-warm)', fontSize: '0.85rem' }}><strong>{step2.packageName}</strong> package</span>
        </div>
        ...
        {/* Menu Summary — no prices, just names */}
        <div className="summary-card">
          <h3 className="summary-card-title">
            <i className="fa-solid fa-bowl-rice" style={{ marginRight: 8 }} />
            Selected Menu <span style={{ color: 'var(--text-muted)', fontSize: '0.82rem' }}>({(step2.items || []).length} items)</span>
          </h3>
          {Object.entries(grouped).map(([cat, items]) => (
            <div key={cat} style={{ marginBottom: 16 }}>
              <p style={{ fontSize: '0.65rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: 8 }}>{cat}</p>
              {items.map(item => (
                <div key={item.id} className="summary-row" style={{ padding: '5px 0' }}>
        ...
        <div className="summary-card">
          ...
          <div className="price-row">
            <span className="lbl">Base Total ({guestCount} guests × ₹{tierPrice}/pp)</span>
      ...
      <div className="booking-fixed-bar">
        <div className="booking-fixed-inner">
          ...
          <button onClick={handleConfirm} disabled={loading} className="btn btn-primary">
            {loading ? <><i className="fa-solid fa-circle-notch fa-spin" /> Processing…</> : <><i className="fa-solid fa-lock" /> Confirm Booking</>}
          </button>
        </div>
      </div>
    </div>
