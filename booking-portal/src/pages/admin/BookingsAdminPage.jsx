  ...
  async function exportExcel() {
    try {
      ...
      const rows = data.bookings.map(b => ({
        'ID': b.id,
        'Customer': b.user?.name || b.guestName || '—',
        'Phone': b.user?.phone || b.guestPhone || '—',
        'Email': b.user?.email || b.guestEmail || '—',
        'Event Type': b.eventType,
        'Event Date': b.eventDate ? format(new Date(b.eventDate), 'dd MMM yyyy') : '—',
        'Guests': b.guestCount,
        ...
        'Status': b.status,
        'Notes': b.adminNotes || '',
        'Booked On': b.createdAt ? format(new Date(b.createdAt), 'dd MMM yyyy') : '—',
      }))
      const ws = XLSX.utils.json_to_sheet(rows)
      ws['!cols'] = [8,20,14,24,14,14,8,28,16,12,12,24,14].map(w => ({ wch: w }))
    ...
    <div>
      <div className="admin-page-header">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={exportExcel} className="btn btn-outline" style={{ fontSize: '0.8rem', padding: '8px 16px', color: '#22c55e', borderColor: '#22c55e' }}>
