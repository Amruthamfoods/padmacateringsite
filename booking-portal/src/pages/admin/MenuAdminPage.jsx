    ...
    (pkg.categoryRules || []).length > 0
      ? pkg.categoryRules.map(r => ({
          ...
          ruleItems: (r.allowedItems || []).map(ai => ({
            ...
            price: ai.premiumPrice ?? ai.menuItem?.price ?? '',
      ...
      for (const rule of rules) {
        ...
        for (const it of rule.ruleItems) {
          ...
          if (it.isNew || !itemId) {
            ...
            /* Always sync price/grams back to menu item base record */
            const patch = {}
            if (it.price !== '' && it.price != null) patch.price = parseFloat(it.price) || 0
            if (it.grams !== '' && it.grams != null) { patch.servingSize = parseFloat(it.grams) || null; patch.servingUnit = it.unit || 'gm' }
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '36px 1fr 80px 60px 52px 28px', gap: 6, alignItems: 'center' }}>
                      ...
                      <div style={{ position: 'relative' }}>
                        <input
                          ...
                          onKeyDown={e => {
                            if (e.key === 'Enter' && it.search.trim() && !it.menuItemId) {
                              e.preventDefault()
                              setItem(ri, ii, 'name', it.search.trim())
                          ...
                          onBlur={() => setTimeout(() => {
                            setItem(ri, ii, 'showDrop', false)
                            // Auto-accept typed name as new item if no match selected
                            if (it.search.trim() && !it.menuItemId) {
                              setItem(ri, ii, 'name', it.search.trim())
                              setItem(ri, ii, 'isNew', true)
                              setItem(ri, ii, 'search', '')
                            }
